...
def handle_ai_round_2():
    ...
    # End AI's turn after shooting
    turn = "player"  # Switch turn to the player

# Other related functions
def handle_magnifier(who_used):
    global magnifier_message, magnifier_display_time, magnifier_bullet_type

    # If no bullets left, just return to prevent errors
    if num_real_bullets == 0 and num_fake_bullets == 0:
        magnifier_bullet_type = None
        return

    # Determine the current bullet based on the available bullets
    bullet_type = None
    if num_fake_bullets > 0 and num_real_bullets > 0:
        bullet_type = bullets[0]  # Use the next bullet in the shuffled bullet list
    elif num_real_bullets > 0:
        bullet_type = 1  # Real bullet if only real bullets are left
    elif num_fake_bullets > 0:
        bullet_type = 0  # Fake bullet if only fake bullets are left
...
def ai_turn():
    ...
    # AI decision-making based on the current round
    if current_round == 1:
        handle_ai_round_1()
...
def render_bullet_info():
    # Render the header with round information
    ...
    # Calculate the Y position for real bullets based on the height of the header
    real_bullets_y = bullet_text_y + bullet_header_surface.get_height() + bullet_line_spacing

    # Render real bullets below the header
    ...
    # Calculate the Y position for fake bullets based on the height of the real bullets text
    fake_bullets_y = real_bullets_y + real_bullets_text.get_height() + bullet_line_spacing

    # Render fake bullets below the real bullets
