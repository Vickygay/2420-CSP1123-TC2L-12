...
lines = [
    "Debtor : Well  ,  guess  thats  all  for  your  end" ,
...
dealer = pygame.transform.scale(dealer, (400, 400))
...
def health_boost():
    ...
    if man.boost_count == 2:
        max_hp = 4
        player_hp = 4  # Increase health to full when 2 boosts are collected
    else:
        max_hp = 3
        player_hp = min(player_hp, max_hp)  # Ensure health doesn't exceed max_hp

# Define for true ending if player got all the boosts in the maze
def true_ending():
    global max_hp, player_hp
    if man.boost_count == 3:  # Check if player has collected all 3 boosts
        max_hp = 4
    ...
    if current_screen == SCREEN_MAIN:
        ...
        while maze_running:
            ...
            if man.is_collision(exit_point):
                if man.boost_count >= 3:  # First condition: 3 boosts
                    SCREENDISPLAY(name)
                    current_screen = SCREEN_PLAY1  # Move to roulette game
                    player_boost_condition = 1  # Set flag for 3 boost condition
                    maze_running = False  # Exit the maze loop
                elif man.boost_count >= 2:  # Second condition: 2 boosts
                    SCREENDISPLAY(name)
                    current_screen = SCREEN_PLAY1  # Move to roulette game
                    player_boost_condition = 2  # Set flag for 2 boost condition
                    maze_running = False  # Exit the maze loop
                else:
        ...
        if current_round == 1:
            ...
            if ai_hp <= 0:  
                player_wins = True  # Set player win condition
                player_game_over = True

                # Check boost conditions to determine which ending to show
                if man.boost_count == 3:
                    current_screen = SCREEN_ENDING2  # True ending (collected 3 boosts)
                elif man.boost_count == 2:
                    current_screen = SCREEN_ENDING3  # Alternate ending (collected 2 boosts)

            # Player loses if their health is 0
            elif player.hp <= 0:  
                player_game_over = True
                current_screen = SCREEN_ENDING1  # Player lost, go to ending 1

            # If player wins and collected 3 boosts, go to true ending
            if player_wins:
                if man.boost_count == 3:
                    true_ending() 
        ...
        screen.fill(BLACK)
        screen.blit(dealer, (player_x, player_y))
